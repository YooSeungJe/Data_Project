...
export default function Admin () {
  ...
  const ReportDropdown = ({ report, handleCategorySelect}) => {
    ...
    return <img src={imageUrl} alt="From Blob" style={{display: 'block', width:' 100%', borderRadius:'0.75rem'}}/>;
    }
    ...
    const handleStatus = async (report, statusString) => {
      ...
      console.log(result);
      alert(`해당 신고는 ${statusString=='completed'?'승인':'거절'}되었습니다`)
      setSelectedItem({});
      SetBlob(null);
    }
      ...
      return (
        ...
        <div className='radio-container'>
          ...
          {asserts.map((assert, index) => (
            <div key={index} className='assert-card' style={{border: assert.id == isCardClicked ? '2px solid black' : 'none'}}>
              <div key={index*100} className='assert-card-content'>
                <h2>신고한 분: {assert.user_id}</h2>
                <h3>욕한 놈: {assert.attacker_id}</h3>
              </div>
              <button className="status-button">{assert.status.charAt(0)}</button>
              <button className={buttonStyles.button} onClick={() => handleItemClick(assert)}>
        ...
        <div className='assert-container content'>
          {selectedItem && (
            <div className='assert-detail-card'>
              ...
              <h2>욕한 놈: {selectedItem.attacker_id}</h2>
              <p>욕 먹은 날짜 :{selectedItem.violence_at}</p>
            ...
            {Object.keys(selectedItem).length !== 0 && (
              <div className='okay-container'>
                <button className='okay completed' onClick={()=>{handleStatus(selectedItem, 'completed'); fetchData();}}>승인</button>
                <button className='okay completed' onClick={()=>{handleStatus(selectedItem, 'rejected'); fetchData();}}>반려</button>
                <button className='okay rejected' onClick={()=>{setSelectedItem({}); SetBlob(null);}}>취소</button>
              </div>
            )}
            </div>
          )}
